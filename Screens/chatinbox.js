...
const Chatinbo = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const nameresponse = await axios.get(`http://${ip}:8000/vendorname/${vendorId}`);
        setVendorName(nameresponse.data.userName);
        const token = await AsyncStorage.getItem("authToken");
        const decodedToken = jwt_decode(token);
        const userloggedId = decodedToken.userId;
        setUserId(userloggedId);

        const response = await axios.get(`http://${ip}:8000/previousmessages/${vendorId}/${userloggedId}`);
    ...
    return (
      <View
        style={[
          ...
          isUserMessage ? styles.userMessageContainer : styles.vendorMessageContainer,
        ]}
      >
        <Text
          style={[
      ...
      <FlatList
        ...
        ListEmptyComponent={() => (
          <View style={styles.noMessagesContainer}>
            <Text style={styles.noMessagesText}>You have no chat with {vendorName}.</Text>
          </View>
        )}
